...
public:
	...
	// mostly used for trigger pins so that they can be used and simulated without a user created input, now also used for struct params
	UPROPERTY(VisibleAnywhere)
	FName AutoGeneratedGraphInputName = NAME_None;

	//for timestamp pins, as they are the only value type that is a struct and doesn't have literal representation which we can read and save
	UPROPERTY()
