...
UZipSoundFactory::UZipSoundFactory(const FObjectInitializer& ObjectInitializer)
...
)
{
	if (FCString::Stricmp(FileType, TEXT("WAV")) == 0)
	{
		...
		if (AudioDeviceManager && ExistingSound)
		{
			...
			{
				UE_LOG(LogTemp, Display, TEXT("Stopping the following AudioComponents referencing sound being imported"));
				for (UAudioComponent* AudioComponent : ComponentsToRestart)
				{
					UE_LOG(LogTemp, Display, TEXT("Component '%s' Stopped"), *AudioComponent->GetName());
					AudioComponent->Stop();
				}
			}
		}
