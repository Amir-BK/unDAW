...
/**
 * A combo box that allows for search filtering of its options
 * based on STextComboBox and SSearchBox
 */
class BKMUSICWIDGETS_API SBKSearchableComboBox : public SComboButton
{
public:
	...
	DECLARE_DELEGATE_RetVal_OneParam(TSharedRef<SWidget>, FOnGenerateWidget, TSharedPtr<FString>);

	DECLARE_DELEGATE_TwoParams(FOnSelectionChanged, TSharedPtr<FString>, ESelectInfo::Type);

	SLATE_BEGIN_ARGS(SBKSearchableComboBox)
		: _Content()
